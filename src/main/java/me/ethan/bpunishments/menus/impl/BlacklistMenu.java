...
public class BlacklistMenu extends PageMenu {

    private final bPunishments plugin = bPunishments.getInstance();
    private final Profile profile;

    public BlacklistMenu(Profile profile) {
    ...
    public String getRawTitle(Player player) {
        return ChatUtils.format("&f" + NameUtils.getName(profile.getUuid()) + "'s &7blacklists");
    }

    private List<Button> getPunishments(Profile profile) {
        List<Button> toButton = new ArrayList<>();
        List<Punishment> bans = profile.getBlacklists();
        bans.forEach(ban -> {
            if(ban.isActive()) {
