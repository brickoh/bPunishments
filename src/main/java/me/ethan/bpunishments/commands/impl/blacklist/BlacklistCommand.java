...
public class BlacklistCommand {
    ...
    @Command(name = "blacklist", aliases = {"blist", "bl"}, permission = "bpunishments.commands.blacklist")
    public void execute(CommandArgs args) {
        ...
        if (sender instanceof ConsoleCommandSender) {
            punishment = new Punishment(Punishment.getNewID(), profile.getUuid(), "CONSOLE", PunishmentType.BLACKLIST, reason, 0L, silent, true);
            punishment.createPunishment();
            profile.getBlacklists().add(punishment);
        } else if (sender instanceof Player player) {
            punishment = new Punishment(Punishment.getNewID(), profile.getUuid(), player.getUniqueId().toString(), PunishmentType.BLACKLIST, reason, 0L, silent, true);
            punishment.createPunishment();
            profile.getBlacklists().add(punishment);
        }
        ...
        if (silent) {
            ...
            bPunishments.getInstance().getRedisManager().write(Payload.STAFF_BLACKLIST_SENT, data);
        } else {
            Bukkit.broadcastMessage(ChatUtils.format(Feedback.GLOBAL_PUNISHMENT_SENT_BLACKLIST)
                    .replace("{offender}", target.getName())
                    .replace("{staff}", sender.getName()));
        }

        if(target.isOnline()) {
            target.getPlayer().kickPlayer(ChatUtils.format(Feedback.KICK_BLACKLIST));
        }
